...
const target = "var tb=document.querySelector('.pipeline-table tbody');if(tb)tb.innerHTML=trows(funded.sort";
const insertBefore = "var tb=document.querySelector('.pipeline-table tbody');";

const statCardUpdate = `// Update stat cards
    var repHeroAmt=document.querySelector('.rep-hero-amount');
    if(repHeroAmt)repHeroAmt.textContent='$'+fmt(vol);
    var repAmts=document.querySelectorAll('.rep-amt');
    for(var ra=0;ra<repAmts.length;ra++){repAmts[ra].textContent='$'+fmt(vol);}
    var cardBigs=document.querySelectorAll('.card-big-single');
...
const newDash = `if(page==='dashboard'){
    ...
    var repHeroAmt=document.querySelector('.rep-hero-amount');if(repHeroAmt)repHeroAmt.textContent='$'+fmt(vol);
