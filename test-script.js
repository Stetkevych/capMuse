  ...
  for(var i=0;i<navEls.length;i++){
    ...
    (function(el){
      el.addEventListener('click',function(e){
        ...
        var all=document.querySelectorAll('.nav-sub-item,.nav-item');
...
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'-')+'</td><td><span class="status-chip '+cc+'">'+cl+'</span></td></tr>';
    }).join('');
  }
 ...
 + fmt(totalAmt) + ' total volume' : '') + '</p><div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden;max-height:72vh;overflow-y:auto"><table class="pipeline-table" style="width:100%"><thead><tr><th style="text-align:left;padding-left:14px">#</th>' + cols.map(c => '<th style="text-align:' + (['amount','revenue','buy_rate','daily_payment','payback','sell_rate','pts'].includes(c)?'right':'left') + '">' + (labels[c]||c) + '</th>').join('') + '</tr></thead><tbody>' + rows.slice(0,100).map((d,i) => {
      return '<tr><td style="padding-left:14px">'+(i+1)+'</td>' + cols.map(c => {
        ...
        if (['amount','revenue','payback','daily_payment'].includes(c)) return '<td style="text-align:right">'+(d[c]?'
</html>
+fmt(d[c]):'-')+'</td>';
...
+fmt(vol);
    ...
    var ft=document.querySelector('.featured-tags');if(ft)ft.innerHTML='<span class="featured-tag">'+funded.length+' Funded</span><span class="featured-tag">
</html>
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'-')+'</td><td><span class="status-chip '+cc+'">'+cl+'</span></td></tr>';
    }).join('');
  }
...
+fmt(vol)+' Volume</span><span class="featured-tag">'+new Set(DEALS.map(function(d){return d.lender;})).size+' Lenders</span>';
...
function trows(rows){return rows.map(function(d,i){var c=stg(d.stage);return'<tr><td>'+(i+1)+'</td><td><div class="biz-cell"><div class="biz-dot" style="background:linear-gradient(135deg,#2563EB,#10B981)">'+ini(d.name)+'</div><span class="biz-cell-name">'+d.name.substring(0,22)+'</span></div></td><td>
</html>
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'-')+'</td><td><span class="status-chip '+cc+'">'+cl+'</span></td></tr>';
    }).join('');
  }
...
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'\u2014')+'</td><td><span class="status-chip '+c[0]+'">'+c[1]+'</span></td></tr>';}).join('');}
function tbl(title,rows,cols){
  ...
  var h='<div><h2 style="font-size:18px;font-weight:700;margin-bottom:6px;color:var(--text-primary)">'+title+'</h2><p style="font-size:13px;color:var(--text-muted);margin-bottom:16px">'+rows.length+' records'+(tot>0?' \u2014 
</html>
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'-')+'</td><td><span class="status-chip '+cc+'">'+cl+'</span></td></tr>';
    }).join('');
  }
...
+fmt(tot)+' volume':'')+'</p><div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden;max-height:72vh;overflow-y:auto"><table class="pipeline-table" style="width:100%"><thead><tr><th style="text-align:left;padding-left:14px">#</th>';
  ...
  rows.slice(0,100).forEach(function(d,i){
    ...
    cols.forEach(function(c){
      ...
      else if(['amount','revenue','payback','daily_payment'].indexOf(c)>-1){h+='<td style="text-align:right">'+(d[c]?'
</html>
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'-')+'</td><td><span class="status-chip '+cc+'">'+cl+'</span></td></tr>';
    }).join('');
  }
