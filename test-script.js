...
function render(page){
  ...
  if(page==='dashboard'){
    ...
    let fm=document.querySelector('.featured-metric');if(fm)fm.textContent='
</html>
...
+fmt(vol);
    ...
    let ft=document.querySelector('.featured-tags');if(ft)ft.innerHTML='<span class="featured-tag">'+funded.length+' Funded</span><span class="featured-tag">
</html>
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'-')+'</td><td><span class="status-chip '+cc+'">'+cl+'</span></td></tr>';
    }).join('');
  }
...
function trows(rows){return rows.map(function(d,i){let c=stg(d.stage);return'<tr><td>'+(i+1)+'</td><td><div class="biz-cell"><div class="biz-dot" style="background:linear-gradient(135deg,#2563EB,#10B981)">'+ini(d.name)+'</div><span class="biz-cell-name">'+d.name.substring(0,22)+'</span></div></td><td>
</html>
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'-')+'</td><td><span class="status-chip '+cc+'">'+cl+'</span></td></tr>';
    }).join('');
  }
...
function tbl(title,rows,cols){
  ...
  let h='<div><h2 style="font-size:18px;font-weight:700;margin-bottom:6px;color:var(--text-primary)">'+title+'</h2><p style="font-size:13px;color:var(--text-muted);margin-bottom:16px">'+rows.length+' records'+(tot>0?' \u2014 
</html>
+fmt(d.amount)+'</td><td>'+(d.lender||d.rep||'-')+'</td><td><span class="status-chip '+cc+'">'+cl+'</span></td></tr>';
    }).join('');
  }
