...
function start(){
  ...
  let dealsLoader=window.CapMuseData?window.CapMuseData.getRawDeals():fx('funding_book_live.json');
  let accountsLoader=isAnalytics?Promise.resolve(null):fx('Accounts.csv');
...
function render(page){
  ...
  if(page==='dashboard'){
    ...
    let fl=document.querySelector('.featured-label');if(fl)fl.textContent='Total Deals - Funding Book';
    let ft=document.querySelector('.featured-tags');if(ft)ft.innerHTML='<span class="featured-tag">Funded</span><span class="featured-tag">Volume</span><span class="featured-tag">Lenders</span>';
