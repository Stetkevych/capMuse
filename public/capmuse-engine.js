...
function render(page){
  ...
  if(page==='dashboard'){
    ...
    var repHeroAmt=document.querySelector('.rep-hero-amount');if(repHeroAmt)repHeroAmt.textContent='
    return;
  }
...
+fmt(vol);
    var repAmts=document.querySelectorAll('.rep-amt');for(var ra=0;ra<repAmts.length;ra++){repAmts[ra].textContent='
    return;
  }
  if(page==='applications'){tbl('Applications',ACCOUNTS,['name','stage','amount','source','applied']);return;}
  if(page==='businesses'){tbl('Businesses',ACCOUNTS.filter(function(d){return d.industry;}),['name','industry','state','revenue','stage']);return;}
...
+fmt(vol);}
    var cardBigs=document.querySelectorAll('.card-big-single');if(cardBigs.length>=1)cardBigs[0].textContent='
    return;
  }
  if(page==='applications'){tbl('Applications',ACCOUNTS,['name','stage','amount','source','applied']);return;}
  if(page==='businesses'){tbl('Businesses',ACCOUNTS.filter(function(d){return d.industry;}),['name','industry','state','revenue','stage']);return;}
...
+fmt(funded.length>0?vol/funded.length:0);if(cardBigs.length>=2)cardBigs[1].textContent=Math.round(funded.length/Math.max(DEALS.length,1)*100)+'%';
