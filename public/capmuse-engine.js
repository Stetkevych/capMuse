...
function start(){
  ...
  for(let i=0;i<navEls.length;i++){navEls[i].style.cursor='pointer';(function(el){el.addEventListener('click',function(e){let href=(this.getAttribute('href')||'').trim();let page=this.getAttribute('data-page')||this.textContent.trim().replace(/[^a-zA-Z]/g,'').toLowerCase();let isHtmlLink=href&&/\.html(\?|#|$)/i.test(href)&&!this.getAttribute('data-page');if(isHtmlLink)return;e.preventDefault();let all=document.querySelectorAll('.nav-sub-item,.nav-item');for(let j=0;j<all.length;j++){all[j].classList.remove('active');}this.classList.add('active');render(page);});})(navEls[i]);}
...
function mapDeal(r){let owner=r.package_owner||r['Owner.name']||r['Package_Owner.name']||'';if(!owner||owner.toLowerCase().indexOf('house')>-1||owner.toLowerCase().indexOf('assigning')>-1)owner=r.puller||r['Puller.name']||'';return{name:r.company||r.Deal_Name||'',stage:r.stage||r.Stage||'',amount:nn(r.funding||r.Funded_Amount),funded_date:r.date_funded||r.Date_Funded||'',applied:r.created_time||r.Created_Time||'',rep:owner,puller:r.puller||r['Puller.name']||'',lender:r.lender||r.Lender||'',source:r.lead_source||r.Lead_Source2||'',industry:r.industry||r.Industry||'',state:r.state||r.State||'',buy_rate:nn(r.buy_rate||r.Buy_Rate),term:r.term||r.Term||'',position:r.position||r.Position||'',daily_payment:nn(r.daily_payment||r.Daily_Payment),payback:nn(r.payback||r.Payback_Amount),usd_cad:r.usd_cad||'',deal_type:r.deal_type||'',revenue:nn(r.revenue),sell_rate:nn(r.sell_rate),points:nn(r.points)};}
...
function render(page){
  ...
  let funded=DEALS.filter(function(d){if(d.funded_date&&d.amount>0)return true;let s=d.stage.toLowerCase();return s.indexOf('won')>-1||s.indexOf('closed')>-1||s.indexOf('fund')>-1;});
  ...
  if(page==='dashboard'){
    ...
    let ft=document.querySelector('.featured-tags');if(ft)ft.innerHTML='<span class="featured-tag">'+funded.length+' Funded</span><span class="featured-tag">$'+fmt(vol)+' Volume</span><span class="featured-tag">'+[...new Set(DEALS.map(function(d){return d.lender;}))].filter(Boolean).length+' Lenders</span>';
